...
### Example: Full Build & Run for B200 (Intranode-only)
...
# 4. Run tuning sweeps (optional, generates plots)
python tests/tune_bf16_sweep.py --num-processes 8   # → /tmp/tune_bf16_sweep.png
python tests/tune_fp8_sweep.py  --num-processes 8   # → /tmp/fp8_graph_all.png  (+ per-config PNGs)

# 5. Re-plot without re-running sweep (if results JSON already exists)
...
3. **Low-latency**: Uses pure RDMA with IBGDA (InfiniBand GPUDirect Async), designed for inference decoding with minimal latency
...
- **NVSHMEM IBGDA**: Required for internode and low-latency; configured via environment variables in `buffer.py` (lines 108-122)
